...
```bash
.venv/bin/python -m pytest tests/ -q       # engine, validation, util, bulk_upload, leave_bulk_upload, lists_bulk_upload, reports, auth, compensation, overtime  → 276 passed
...
| `app/routes/` | `auth` (login/signup), `employee` (Today/My Month/Leave/Overtime/Support/Profile), `admin` (dashboard/roster/lists/leave/overtime/config/audit/support), `reports`, `exports` |
...
| `tests/` | 276 tests: engine, validation, util, bulk_upload, leave_bulk_upload, lists_bulk_upload, reports, auth, compensation, overtime |
