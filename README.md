...
| Holidays | `/holidays` | Behind `HOLIDAY_MANAGEMENT_ENABLED` (on by default — see the feature-flag note above). US/India tabs (Ganesh, 2026-08-12 — the team now has both) showing that country's holiday list; defaults to the employee's own Profile country, either tab is viewable regardless. Informational only — the compliance calendar (My Month) always uses the employee's own `location`, never whichever tab is open here. |
...
| Profile | `/profile` | A **Country** dropdown (US/India, Ganesh 2026-08-12; behind `HOLIDAY_MANAGEMENT_ENABLED`, on by default) next to the photo upload — self-service, takes effect immediately, and decides which country's Holiday calendar drives that employee's own My Month/compliance days (`app/engine.py`'s `holidays_set()`/`is_working_day()`, both now `Employee.location`-scoped). Upload/replace a profile photo (JPEG/PNG/WebP, ≤2 MB). Two more cards: **Personal Details** (`/profile/personal-details` — DOB, blood type, gender, marital status, family, nationality, hobbies/skills/languages, plus every contact number and address in the same form) and **Employment Details** (`/profile/employment-details` — bank account + PAN/Aadhaar/UAN/ESI; sensitive numbers are masked to last-4 everywhere, including to admins, the instant they're saved). Both are self-service and optional. |
...
| Suggestions | `/admin/suggestions` | Approve/reject Project/Task suggestions employees or leads have submitted from Today. **Department-scoped admins** see only suggestions from their own team (scoped by the *submitter's* department); Super Admins see everything. A suggestion is invisible and unusable by anyone — including whoever suggested it — until approved here (Ganesh, 2026-08-11); approving makes it visible/usable to everyone, rejecting also deactivates it. |
...
| Holiday Management | `/admin/holidays` | **Super-Admin-only.** Under Settings & Configurations (Ganesh, 2026-08-12). Separate US and India sections — each employee's own `location` decides which one their compliance calendar/My Month actually uses (`app/engine.py`'s `holidays_set()`/`is_working_day()`). Add/delete per country, plus a **bulk upload** (`/admin/holidays/bulk-upload`, `app/holiday_bulk_upload.py`) taking Holiday Name / Holiday Date / Country columns — re-uploading a row whose date+country already exists updates its name instead of duplicating it. |
...
| `app/engine.py` | Status/variance/strike computation, recompute, ledger, `today_attendance()`, `leave_balance()`, `holidays_set()`/`holidays_by_location()` (per-country, 2026-08-12) |
...
- `test_engine.py` — status mapping incl. exact tolerance boundaries, leave-reduced targets, weekend surplus, part-time targets, strike counting, compensation on/off, override precedence, pre-policy exemption, and (Ganesh, 2026-08-12) per-country holidays — `holidays_set()`/`holidays_by_location()` scoping, an employee still working on the *other* country's holiday, and an end-to-end `recompute_employee()` check that a day only reads HOLIDAY for the matching country.
- `test_validation.py` — overlaps, touching rows, midnight/duration/details rules, locked days, deactivated dropdown values, back-dating across weekends and holidays, gap flags (Ganesh, 2026-08-11: now netting out overlapping break time before thresholding, and blocking a new row from being logged over a break in the first place), and (Ganesh, 2026-08-01, tightened 2026-08-11) pending Project/Task suggestions unusable by anyone — including whoever submitted them — until approved, rejected suggestions unusable even by the submitter.
- `test_util.py` — formatting/parsing helpers, including `role_to_flags`/`flags_to_role` (the three-tier admin role mapping), `ensure_bootstrap_admins` (creates the initial Super Admins on a fresh deploy only, permanent no-op once any employee exists), and `punch_out_error` (Ganesh, 2026-08-11 — Punch Out blocked until the day's task log is Submit Day'd).
...
- `test_holiday_bulk_upload.py` — Country parsing (case-insensitive, must be US/India), date parsing (text or Excel date cells), re-uploading a row whose date+country already exists updates the name instead of creating a duplicate, same date valid as two separate rows when the country differs, sample/export workbook round-trips.
